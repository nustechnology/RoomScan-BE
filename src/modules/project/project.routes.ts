...
function projectModuleMapError(error: unknown): AppError | undefined {
...
export function createProjectRouter({
  ...
  router.post(
    ...
    route(async (request, response) => {
      ...
      const result = await projectService.createIdempotently(userId, body, key);
  ...
  router.patch(
    ...
    route(async (request, response) => {
      ...
      const result = await projectService.update(userId, params.projectId, expectedRevision, data);
      const responseBody = ProjectResponseSchema.parse(result);
