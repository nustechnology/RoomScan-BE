...
the current user as usual. No route currently mounts it: invitation preview,
which it was introduced for, requires authentication so it can report whether
the current user already has access and enforce the recipient binding.
...
resource (`projectId` or `scanId`): its recipient binding, `tokenHash`
...
`{INVITATION_BASE_URL}/invitations/{rawToken}`.

The recipient binding is exactly one of `recipientUserId` (a foreign key to
`User`) or `recipientEmail`, enforced in the database by the
`invitations_recipient_channel_check` constraint. `recipientUserId` is the
reliable form and is how the app addresses invitations: the Owner types the
recipient's `publicUserId`, the service resolves it to an account, and
acceptance then requires the signed-in user to _be_ that account. The email form
exists because someone who has not signed up yet has no public user id; it is
bearer-style, exactly like a `ShareLink`, since the address an Owner types has
no relationship to the account the recipient signs in with (Apple's Hide My
Email in particular issues the account a different address). Comparing the
signed-in user's email against the invited address rejected the legitimate
recipient in that case, so no such comparison is made.

`User.publicId` is a random, unique ten-character identifier over
`23456789ABCDEFGHJKMNPQRSTVWXYZ` (generated by
`src/common/identifiers/public-user-id.ts`, which omits the characters people
confuse when reading an id aloud). It is assigned at user provisioning, retried
on the unique-index collision, and backfilled for pre-existing users inside the
`add_public_user_id_and_invitation_recipient_user` migration so the column is
`NOT NULL` from the moment it exists.

Creating an invitation sends an AC5-style email built by `buildInvitationEmail`
with either the project or the scan template; a delivery failure is logged and
never fails the request, and a user-bound recipient with no address on file
simply has the send skipped and logged. Partial unique indexes on
`(projectId, recipientEmail)`, `(scanId, recipientEmail)`,
`(projectId, recipientUserId)`, and `(scanId, recipientUserId)` for `PENDING`
rows enforce at most one pending link per recipient and scope: the repository
create revokes any expired pending link for the same recipient and scope and
inserts the new link in one database transaction, so a concurrent duplicate
raises the unique-constraint violation and is mapped to
`409 INVITATION_ALREADY_SENT`. An expired link therefore does not block
re-inviting the recipient. Resending rotates the token, extends `expiresAt`,
updates `sentAt`, re-reads a user-bound recipient's current address, and
re-sends the email with the matching scope template.

`GET /api/v1/invitations` is the recipient-side inbox: it lists the `PENDING`
invitations bound to the signed-in user, so an invitation stays reachable even
when the email never arrives. Preview, accept, and decline therefore take a
_reference_ that is either the 43-character link token or — only for an
invitation bound to the caller — that invitation's id. An invitation id is not a
secret (the Owner sees it in the share list), so resolving one is scoped to the
bound recipient and every other id returns `404 INVITATION_NOT_FOUND`.
...
Acceptance is open to any signed-in holder of the token for an email-addressed
invitation and restricted to the bound user for a user-addressed one
(`403 INVITATION_NOT_FOR_USER` otherwise). The first user to redeem a pending
...
Preview (`GET /invitations/:reference`) requires a Bearer token and resolves
...
matching entity (`project` or `scan`), the link status, how the invitation was
addressed (`recipientEmail` / `recipientPublicUserId`, exactly one non-null),
and `hasAccess` for the calling user. A project is only
...
them — `POST /invitations/:reference/accept` shares `/invitations/:reference` with the
