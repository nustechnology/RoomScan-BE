...
| [Client invitation guide](client-invitation-guide.md)   | How the mobile client consumes the invitation surface: public user ids, the two ways to address an invitation, the inbox, and the reference-based preview/accept path | Changing the invitation HTTP contract, the recipient model, or anything the client must change to keep the sharing flow working |
| [Development workflow](development.md)                  | Docs-first task loop, local setup, native quality gates, database-only Docker usage, Prisma migrations, and generated files                                           | Changing scripts, tooling, tests, CI, hooks, environment variables, Docker, Prisma workflow, or developer operations            |
